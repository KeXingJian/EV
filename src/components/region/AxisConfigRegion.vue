<template>
  <div class="axis-container">
    ...
    <div class="options">
      <div>
        <div class="axis"v-for="item in loadedHAxis"
             :key="item.id"
        >
          ...
          <div class="axis-body">
            <div>
              <AxisConfig :item="item" :type="0"></AxisConfig>
            </div>
          </div>
        </div>
        <div class="axis"v-for="item in loadedVAxis"
             :key="item.id"
        >
          <div class="axis-header" @click="toggleDataset($event)">
            <div class="axis-header-left">
          ...
          <div class="axis-body">
            <div>
              <AxisConfig :item="item" :type="1"></AxisConfig>
            </div>
          </div>
        </div>
      </div>
...
import AxisConfig from "../config/AxisConfig.vue";
...
const {Hs,Vs} = storeToRefs(useOptionConfig())

const loadedHAxis =computed(()=>{
  return Hs.value.filter(i=>i.isLoad)
})
const loadedVAxis =computed(()=>{
  return Vs.value.filter(i=>i.isLoad)
})
</script>
