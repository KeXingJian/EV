...
const loadX0yAxis = (c, echartsOptions) => {
    ...
    echartsOptions.xAxis.push(xAxis)
    echartsOptions.yAxis.push(yAxis)
    echartsOptions.grid.push(grid)
...
const loadPolarAxis = (c, echartsOptions) => {
    ...
    echartsOptions.radiusAxis.push(R)
    echartsOptions.angleAxis.push(A)
    echartsOptions.polar.push(polar)
...
export const changeAxisType = (c, echartsOptions) => {
    ...
    if (c.type === 0) {
        ...
        const targetXAxis = echartsOptions.xAxis.find(i => i.id === c.id)
        const targetYAxis = echartsOptions.yAxis.find(i => i.id === c.id)
        ...
        const targetXAxis = echartsOptions.radiusAxis.find(i => i.id === c.id)
        const targetYAxis = echartsOptions.angleAxis.find(i => i.id === c.id)
...
export const deleteAxis = (c, Cs, Ss, echartsOptions) => {
    if (c.type === 0) {
        echartsOptions.xAxis = echartsOptions.xAxis.filter(i => i.id !== c.id)
        echartsOptions.yAxis = echartsOptions.yAxis.filter(i => i.id !== c.id)
        echartsOptions.grid = echartsOptions.grid.filter(i => i.id !== c.id)
    } else if (c.type === 1) {
        echartsOptions.radiusAxis = echartsOptions.radiusAxis.filter(i => i.id !== c.id)
        echartsOptions.angleAxis = echartsOptions.angleAxis.filter(i => i.id !== c.id)
        echartsOptions.polar = echartsOptions.polar.filter(i => i.id !== c.id)
    }



    echartsOptions.graphic.elements = echartsOptions.graphic.elements.filter(i=>{
        return i.key !== 'resizerRect'+c.id
    })


    Ss.filter(i => i.C?.id === c.id).forEach((i) => {
        if (i.isLoad) unloadSeriesLazy(i, echartsOptions)
        i.C = null
    })

    Cs.splice(Cs.indexOf(i => i.id === c.id))
